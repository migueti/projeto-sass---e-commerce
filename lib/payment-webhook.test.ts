...
describe("payment webhook", () => {
  ...
  it("rejects malformed or duplicated signature fields", () => {
    const secret = "webhook-secret";
    const dataId = "payment-123";
    const requestId = "request-456";
    const timestamp = "1710000000";
    ...
    expect(webhookSignatureIsValid(`ts=${timestamp},ts=${timestamp},v1=${hash}`, requestId, dataId, secret, 1710000000)).toBe(false);
    expect(webhookSignatureIsValid(`ts=${timestamp},v1=${hash}=extra`, requestId, dataId, secret, 1710000000)).toBe(false);
    expect(webhookSignatureIsValid(`ts=${timestamp},v1`, requestId, dataId, secret, 1710000000)).toBe(false);
  });
